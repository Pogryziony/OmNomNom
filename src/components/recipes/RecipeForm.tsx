...
import {
  buildCreateRecipeCommand,
  buildIngredientInputs,
  recipeDtoToFormValues,
...
export default function RecipeForm(props: Props) {
  ...
  useEffect(() => {
    ...
    if (!accessToken) {
      setLoadState({
        kind: "error",
        message: "Login required to edit recipes.",
      });
      return;
    }
    ...
    fetchJson<RecipeDTO>(recipeUrl, {
      ...
      .catch((err) => {
        ...
        setLoadState({
          kind: "error",
          message: err instanceof Error ? err.message : "Failed to load recipe",
        });
      });
  ...
  function updateIngredientRow(
    index: number,
    patch: Partial<(typeof ingredients)[number]>,
  ) {
    setIngredients((rows) =>
  ...
  function validate(
    command: CreateRecipeCommand | UpdateRecipeCommand,
  ): string | null {
    if (!command.title || command.title.trim().length === 0)
      return "Title is required.";
    if (!command.instructions || command.instructions.trim().length === 0)
      return "Instructions are required.";
    if (!command.servings || command.servings <= 0)
      return "Servings must be greater than 0.";
    if (command.description && command.description.length > 250)
      return "Description cannot exceed 250 characters.";
    if (
      command.prep_time !== undefined &&
      command.prep_time !== null &&
      command.prep_time < 0
    ) {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    const command = buildCreateRecipeCommand({
      ...
      servings,
      prep_time:
        prepTime.trim().length === 0 ? null : Number.parseInt(prepTime, 10),
      ingredients,
    });
    ...
    try {
      ...
      const updated = await fetchJson<RecipeDTO>(
        `/api/recipes/${props.recipeId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
        ...
        {!authLoading && !accessToken ? (
          <div className="space-y-2">
            <p className="text-gray-800">
              You need to log in to manage recipes.
            </p>
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-600">
              Optional. Used for tiles and the recipe page.
            </p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={servings}
              onChange={(e) =>
                setServings(Number.parseInt(e.target.value || "0", 10) || 0)
              }
            />
          </div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              ...
              >
                <input
                  ...
                  disabled={uploadState === "uploading" || !accessToken}
                  onChange={(e) =>
                    onInstructionFileSelected(e.target.files?.[0] ?? null)
                  }
                />
                Upload step photo
              </label>
              <span className="text-xs text-gray-600">
                Appends a Markdown image to the instructions.
              </span>
            </div>
          </div>
