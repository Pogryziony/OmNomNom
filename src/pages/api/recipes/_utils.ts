...
export async function getOrCreateIngredient(
  ...
  if (selectError) {
    throw new Error(
      `Failed to look up ingredient "${normalizedName}": ${selectError.message}`,
    );
  }
  ...
  if (insertError) {
    ...
    throw new Error(
      `Failed to insert ingredient "${normalizedName}": ${insertError.message}`,
    );
  }

  if (!inserted) {
    throw new Error(
      `No data returned from ingredient insert for "${normalizedName}"`,
    );
  }
