...
async function upsertIngredients(
  ...
  for (const input of inputs) {
    ...
    const data = await getOrCreateIngredient(
      supabase,
      normalizedName,
      displayName,
    );
...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let query = locals.supabase
      .from("recipes")
      .select(
        "id, title, description, servings, prep_time, cook_time, image_url, is_public, created_at, updated_at",
        { count: "exact" },
      )
      .eq("user_id", user.id);
    ...
    if (queryError) {
      ...
      return jsonError("INTERNAL_ERROR", "Failed to fetch recipes", 500);
    }
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (recipeError || !recipe) {
      ...
      return jsonError("INTERNAL_ERROR", "Failed to create recipe", 500);
    }
    ...
    // @ts-ignore - Database types not yet generated from schema
    const { data: fullIngredients, error: ingredientsError } =
      (await locals.supabase
        .from("ingredients")
        .select("*")
