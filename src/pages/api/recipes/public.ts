...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let query = locals.supabase
      .from("recipes")
      .select(
        "id, title, description, servings, prep_time, cook_time, image_url, published_at, user_id",
        { count: "exact" },
      )
      .eq("is_public", true);
    ...
    if (queryError) {
      ...
      return jsonError("INTERNAL_ERROR", "Failed to fetch public recipes", 500);
    }
    ...
    const recipeCountsMap = new Map(
      recipeCountsResults.map((r) => [r.userId, r.count]),
    );
