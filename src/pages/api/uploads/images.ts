...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    console.error(
      "Failed to parse multipart/form-data in /api/uploads/images",
      err,
    );
  ...
  const uploadResult = await locals.supabase.storage
    .from(bucket)
    .upload(objectPath, maybeFile, {
  ...
  if (uploadResult.error) {
    ...
    const statusCode =
      typeof anyError.statusCode === "string"
        ? Number(anyError.statusCode)
        : anyError.statusCode;
    const message =
      anyError.message || anyError.error || "Failed to upload image";
    ...
    const isRlsViolation =
      statusCode === 403 ||
      /row\s+level\s+security|violates\s+row\s+level\s+security|rls/i.test(
        message,
      );
  ...
  const publicUrlResult = locals.supabase.storage
    .from(bucket)
