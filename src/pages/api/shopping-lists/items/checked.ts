...
export const DELETE: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (countError) {
      ...
      return jsonError("INTERNAL_ERROR", "Failed to count checked items", 500);
    }
    ...
    if (deleteError) {
      ...
      return jsonError("INTERNAL_ERROR", "Failed to delete checked items", 500);
    }
    ...
    console.error(
      "DELETE /api/shopping-lists/items/checked unexpected error:",
      error,
    );
